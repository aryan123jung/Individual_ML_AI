...
BATTING_RAW = "data/processed/smat_batting_raw.csv"
BOWLING_RAW = "data/processed/smat_bowling_raw.csv"
JSON_DIR = "data/raw/Syed_Mushtaq_Ali_Trophy_Raw"
MIN_BAT_MATCHES = 3
MIN_BOWL_MATCHES = 3

OUTPUTS = {
    "batting": "data/processed/smat_batting_features_v2.csv",
    "bowling": "data/processed/smat_bowling_features_v2.csv",
    "allrounder": "data/processed/smat_allrounder_features_v2.csv",
    "positions": "data/processed/smat_batting_positions_v2.csv",
}
...
def compute_batting_features(df):
    ...
    features["duck_count"] = g.apply(
        lambda x: ((x["runs"] == 0) & (x["dismissed"] > 0)).sum(),
    ...
    features["innings_above_30"] = g["runs"].apply(lambda x: (x >= 30).sum())
    features["innings_above_50"] = g["runs"].apply(lambda x: (x >= 50).sum())
...
def compute_bowling_features(df):
    ...
    features["two_plus_hauls"] = g["wickets"].apply(lambda x: (x >= 2).sum())
